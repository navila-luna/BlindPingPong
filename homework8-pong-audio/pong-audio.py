...
player2_cmd = 0
pitch = 0
MIN_PITCH = 250 #just because this min pitch fits my pitch range   #200
MAX_PITCH = 460 # 380
INTERVALS = 7
song = None
...
hit()
def talk():
    os.system("say \"Welcome To Blind-Folded Ping Pong\"")
    os.system("say \"Player 1 controls the left paddle and uses their voice. To move paddle up, make a high pitch, to move paddle down make a lower pitch.\"")
    os.system("say \".. Player 2 controls the right paddle and uses the keyboard to move. To move paddle UP, type O. To move paddle down, type L.\"")
    os.system("say \" To quit, say QUIT. To replay these instructions say [[emph +]] replay\"")
talk()
...
class Player(object):
    ...
    def __init__(self, NUMBER, screen_WIDTH=800):
        ...
        #listen_to_speech() 
        if NUMBER == 0:
           #print(NUMBER, self.up_key, self.down_key)
...
class Model(object):
    ...
    def check_if_paddled(self): 
        ...
        if cross0 and -25 < b.y - p0.y < 25:
           # playhit = threading.Thread(target=hit(), args=()) # I uncommented this 
           # playhit.start()
            hit()
...
#-----------------------------------------------------
    # Pitch frequency based on distance from the ball
    # call the ball distance from left side
    # the higher the number the closer it is to the player
    # if player the direction is to player 1 (left) then play audio 
    def ball_distance(self, pitch, volume):
        # if pitch is still 0
        global song
        song = AudioSegment.from_wav("./audioFiles/pitch" + str(int(pitch))+ ".wav")
        sound_thread = threading.Thread(target=play, args=(song[:200] - volume + 7,))
...
# -------------- Ball position: you can find it here -------
    ...
    # both are according to balls proximity to the paddle
    # smaller the y (vertical/up) then the higher pitch the sound is
    # gives you the bounds 
        if self.i % 10 == 0:
            print(b.y)
            print(b.y // (self.HEIGHT / INTERVALS))
            volume = ((b.x/self.WIDTH)) * 25
            self.ball_distance(b.y // (self.HEIGHT / INTERVALS), volume)
    def update(self):
        ...
        # player 1: uses their voice
        p1 =  self.players[0]
        global player1_cmd  
        if 50 <= pitch <= 1000:   
            currPitch = MIN_PITCH + ((MAX_PITCH - MIN_PITCH)*(1 - (p1.y/ self.HEIGHT)))
            if pitch < currPitch:
                p1.y += 1 + self.speed
            if pitch > currPitch:
                p1.y -= self.speed
